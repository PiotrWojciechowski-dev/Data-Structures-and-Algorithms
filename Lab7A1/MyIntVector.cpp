...
MyIntVector::MyIntVector(MyIntVector&& other) noexcept : m_size(0), m_capacity(0), arrPtr(nullptr) {
}
...
MyIntVector& MyIntVector::operator=(MyIntVector&& rhs) {
	rhs.swap(*this);
	return *this;
}


void MyIntVector::swap(MyIntVector& second) {

	using std::swap;
	swap(m_size, second.m_size);
	swap(m_capacity, second.m_capacity);
}